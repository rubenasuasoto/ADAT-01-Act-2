...
*/
        ...
        br.use {
            it.forEachLine { line ->
                ...
                val map = mutableMapOf(
                    Pair(first = "Apellido", second = todoSpliteado.get(0)),
                    Pair("Nombre", todoSpliteado.get(1)),
                    Pair("Asistencia", todoSpliteado.get(2).replace("%", "")),
                    Pair("Parcial1", todoSpliteado.get(3).replace(",", ".")),
                    Pair("Parcial2", todoSpliteado.get(4).replace(",", ".")),
                    Pair("Ordinario1", todoSpliteado.get(5).replace(",", ".")),
                    Pair("Ordinario2", todoSpliteado.get(6).replace(",", ".")),
                    Pair("Practicas", todoSpliteado.get(7).replace(",", ".")),
                    Pair("OrdinarioPracticas", todoSpliteado.get(8).replace(",", ".")),
    ...
    fun calcularNotaFinal(todoSpliteado : List<String>):String {
